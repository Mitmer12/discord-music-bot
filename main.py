        ...
        async with ctx.typing():
            ...
            if 'open.spotify.com' in search:
                ...
                if ctx.voice_client.is_playing() or queue:
                    ...
                    # İlk şarkıyı çal, geri kalanları sıraya ekle
            ...
            # YouTube şarkı işleme
            if ctx.voice_client.is_playing() or queue:
                queue.append(search)
                # Şarkı bilgisini almaya çalış
                try:
...
async def efekt(ctx, effect=None):
    ...
    try:
        ...
        if effect in available_effects:
            ...
            # Eğer şarkı çalıyorsa yeniden başlat
...
async def history(ctx):
    ...
    try:
        ...
        # Son 10 şarkıyı göster (tersine çevrilmiş)
...
async def geriekle(ctx, index: int = 1):
    ...
    try:
        ...
        if 1 <= index <= len(history):
            # Geçmişten şarkı al (tersten sayma)
            song = list(history)[-(index)]
            
            # Şarkı adını almaya çalış
            try:
            ...
            # Eğer hiçbir şey çalmıyorsa ve sıra boşsa direkt çal
            if not ctx.voice_client.is_playing() and not queue:
                async with ctx.typing():
                    ...
                    current_songs[guild_id] = player
                    history.append(song)  # Geçmişe tekrar ekle (en sona)
...
async def stop(ctx):
    ...
    try:
        if ctx.voice_client:
            ...
            queue = get_queue(guild_id)
            queue.clear()
            repeat_modes[guild_id] = 0  # Tekrarı kapat
            ctx.voice_client.stop()
            if guild_id in current_songs:
                del current_songs[guild_id]
